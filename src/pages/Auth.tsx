 ...
 * InvalidSecret\n at ... Called by client". Pull out just the real message —
 * and if nothing useful is in there, return a friendly fallback instead of
 * ever showing the raw envelope to the user.
 */
function friendlyAuthError(error: unknown): string {
  const raw = error instanceof Error ? error.message : "";
  if (!raw) return "Something went wrong. Please try again.";

  // Innermost "Uncaught Error: <message>" / "ConvexError: <message>".
  const innerMatch = raw.match(/(?:Uncaught\s+)?(?:ConvexError|Error):\s*([^\n]+)/);
  const candidates = [innerMatch?.[1]?.trim() ?? "", raw.split("\n")[0].trim()];
...
function Auth({ redirectAfterAuth }: AuthProps = {}) {
      ...
      if (message.includes("already exists")) {
        switchToLogin(
          method === "phone"
            ...
            : "This email is already registered — sign in below with your password.",
        );
      } else if (
        ...
        setError(
          isSignup
            ...
            : method === "phone"
              ? "Wrong number or password. Check and try again."
        ...
        message.toLowerCase().includes("valid phone") ||
  ...
  const description = isPhone
    ...
    : isSignup
      ? "Your email + a password — no OTP needed"
                    ...
                    <p className="mt-2 text-center text-[11px] leading-5 text-slate-400 dark:text-slate-500">
                      {isSignup
                        ...
                        : isPhone
                          ...
                          : "Use the same email and password every time — no codes to wait for."}
                    </p>
                  </form>
                </CardContent>
